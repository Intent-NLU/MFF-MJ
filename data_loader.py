...
from utils import get_intent_labels, get_slot_labels,change_intent_labels
import utils
...
class JointProcessor(object):
    ...
    def get_examples(self, mode):
        ...
        logger.info("LOOKING AT {}".format(data_path))
            ...
            #intent单独处理
        with open(os.path.join(data_path, self.intent_label_file), "r", encoding="utf-8") as f:
            intents = []
            for line in f:
                l=line.split("#")
...
def convert_examples_to_features(args,examples, max_seq_len, tokenizer,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        intent_label_id=example.intent_label
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    if os.path.exists(cached_features_file):
        ...
        if mode == "test":
            if args.task=="MixATIS":
                processor.intent_labels=change_intent_labels(args)
            examples = processor.get_examples("test")
        elif mode == "dev":
            if args.task=="MixATIS":
                processor.intent_labels=get_intent_labels(args)
    ...
    if mode=="test" and args.only_eval:
        if args.task == "MixATIS":
            processor.intent_labels = change_intent_labels(args)
    ...
    intent_label_lst = len(get_intent_labels(args))
    intent_one_hot = [utils.multilabel2one_hot(f.intent_label_id, intent_label_lst) for f in features]
    all_intent_label_ids= torch.Tensor(intent_one_hot)
