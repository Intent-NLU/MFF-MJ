...
from utils import MODEL_CLASSES, get_intent_labels, get_slot_labels,multi_compute_metrics
...
class Trainer(object):
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    if self.args.logging_steps > 0 and global_step % self.args.logging_steps == 0:
                        ...
                        if result["sementic_frame_acc"]>max_overall_acc and result["slot_f1"]>max_slot_f1:
                            self.save_model()
    ...
    def evaluate(self, mode):
        ...
        total_result=multi_compute_metrics(intent_preds, out_intent_label_ids, slot_preds_list, out_slot_label_list,pred_intent_acc,real_intent_acc)
    ...
    def load_model(self):
        ...
        try:
            #self.model=torch.load("MixATIS_model/pytorch_model.bin",map_location='cpu')
            self.model = self.model_class.from_pretrained(self.args.model_dir,
                                                          args=self.args,
                                                          intent_label_lst=self.intent_label_lst[0:17],
                                                          slot_label_lst=self.slot_label_lst)
            #self.model.to(self.device)
...
#        self.model.eval()
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                outputs, i_preds, intent_process = self.model(**inputs)
                # print('intent_process:',type(intent_process),intent_process[0])

                # use process code
                for i in intent_process:
                    temp = []
                    for j in i:
                        temp.append(self.intent_label_lst[j])
                    pred_intent_acc.append(temp)
            ...
            if intent_preds is None:
                ...
                intent_preds = i_preds
                out_intent_label_ids = inputs['intent_label_ids'].detach().cpu().numpy()
            else:

                intent_preds = np.append(intent_preds, i_preds.detach().cpu().numpy(), axis=0)
